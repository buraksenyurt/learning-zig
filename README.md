...
- **comptime**, derleme zamanında mutlaka bilinmesi gereken türler için kullanılan bir kavram olarak düşünülebilir. **Sıfır boyutludur** ve çalışma zamanında kullanılamazlar. **comptime_int**, **comptime_float** gibi varyasyonları vardır. **generic** türde fonksiyonlar yazılırken, metadata programlamada kullanılırlar. Kritik nokta comptime olarak tanımlanmış bir değişkenin alacağı değerin derleme zamanında bilinmesi gerektiğidir. Buna göre çalışma zamanında kullanıcıdan alınan bir veri, comptime olarak tanımlanmış bir değişkenle ilişkilendirilemez, derleme zamanı hatası oluşur.
